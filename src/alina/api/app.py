...
def create_app():
    ...
    app=FastAPI(title="ALINA",version="0.1.0",description="Actionable Leadership Intelligence for Navigation & Alignment")
    ...
    def get_situation(sid: str):
        ...
        if item is None: raise HTTPException(status_code=404,detail="Situation not found")
        return item

    @app.get("/v1/patterns", response_model=PatternReport)
    def patterns(days: int=Query(30,ge=1,le=3650)):
        repo=SituationRepository(db); return build_pattern_report(repo.list(limit=500,days=days),window_days=days)
